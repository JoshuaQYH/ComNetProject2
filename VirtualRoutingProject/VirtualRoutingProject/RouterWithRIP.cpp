...
void RouterWithRIP::CheckRouterTable() {
	cout << LOCAL << "$: ";
...
bool RouterWithRIP::CheckLinkState(int RouterID) {
	for (auto i = RouterTable.begin(); i != RouterTable.end(); i++) {
...
bool RouterWithRIP::UpdateForwardingTable(vector<ForwardingRouterInfo>& fromOtherRouterTable, const int fromNum) {
...
bool RouterWithRIP::ReceiveUpdatingRouterInfo(vector<ForwardingRouterInfo>& fromOtherRouterTable, const int fromNum) {
	return UpdateForwardingTable(fromOtherRouterTable, fromNum);
}

bool RouterWithRIP::UpdateForwardingTable(const int nextNum, const int st, const int cost) {
	cout << LOCAL << "$: ";
	bool hasChanged = false;
...
vector<ForwardingRouterInfo>* RouterWithRIP::SendUpdatingRouterInfo(vector<ForwardingRouterInfo>* toOtherRouterTable) {
	for (auto i = ForwardingTable.begin(); i != ForwardingTable.end(); i++) {
		ForwardingRouterInfo temp;
...
bool RouterWithRIP::ModifyNeighborRouterInfo(int neighborRouterID, int cost) {
	for (auto i = RouterTable.begin(); i != RouterTable.end(); i++) {
...
int RouterWithRIP::DecideForwardingAddress(int routerID) {
	cout << LOCAL << "$: ";
	for (auto i = ForwardingTable.begin(); i != ForwardingTable.end(); i++) {
...
void RouterWithRIP::PrintForwardingTable() {
	cout << LOCAL << "$: " << endl;
	for (auto i = ForwardingTable.begin(); i != ForwardingTable.end(); i++) {
...
void RouterWithRIP::DrawRouterTable() {
	cout << LOCAL << "$: ";
